...
> This is Part II of a two-part study. The full write-up, alongside the
> randomized optimization half, lives on the project page:
...
- The optimal number of clusters (determined by silhouette analysis and the elbow method) was **$K=5$ to $7$** for the stock market dataset. On the tennis dataset the silhouette scores never separated cleanly enough to name a comparable range, which is consistent with the negative scores EM produced there.
